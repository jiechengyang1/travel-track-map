...
export default function MapContainer({
  ...
  const clearOverlayRefs = () => {
    routeLineRefs.current.forEach((overlay) => overlay.setMap?.(null));
    labelMarkerRefs.current.forEach((overlay) => overlay.setMap?.(null));
    nodeMarkerRefs.current.forEach((overlay) => overlay.setMap?.(null));
  ...
  }, [mapLoaded, currentMapStyle, tripData, segmentStatuses, flattenedRoute]);
  ...
  }, [activeNode, tripData.previewNodes, mapLoaded]);
  ...
  useEffect(() => {
    ...
    if (followVehicle) {
      ...
      const movedFarEnough = !lastCenter || Math.abs(lastCenter[0] - travelerCoord[0]) > 0.02 || Math.abs(lastCenter[1] - travelerCoord[1]) > 0.02;
